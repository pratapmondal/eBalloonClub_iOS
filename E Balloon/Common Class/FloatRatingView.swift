    ...
    */
    ...
    func refresh() {
        for i in 0..<self.fullImageViews.count {
            ...
            if self.rating>=Float(i+1) {
                ...
                maskLayer.frame = CGRect(x: 0, y: 0, width: CGFloat(self.rating-Float(i))*imageView.frame.size.width, height: imageView.frame.size.height) //CGRectMake(0, 0, CGFloat(self.rating-Float(i))*imageView.frame.size.width, imageView.frame.size.height)
    ...
    func sizeForImage(image: UIImage, inSize size:CGSize) -> CGSize {
        ...
        if imageRatio < viewRatio {
            ...
            return CGSize(width: width, height: size.height) //CGSizeMake(width, size.height)
        }
        else {
            let scale = size.width / image.size.width
            let height = scale * image.size.height
            
            return CGSize(width: size.width, height: height) //CGSizeMake(size.width, height)
        }
    }
    ...
    override public func layoutSubviews() {
        ...
        if let emptyImage = self.emptyImage {
            ...
            let imageViewSize = self.sizeForImage(image: emptyImage, inSize: CGSize(width: maxImageWidth, height: maxImageHeight)) //CGSizeMake(maxImageWidth, maxImageHeight))
            ...
            for i in 0..<self.maxRating {
                let imageFrame = CGRect(x: i==0 ? 0:CGFloat(i)*(imageXOffset+imageViewSize.width), y: 0, width: imageViewSize.width, height: imageViewSize.height) //CGRectMake(i==0 ? 0:CGFloat(i)*(imageXOffset+imageViewSize.width), 0, imageViewSize.width, imageViewSize.height)
